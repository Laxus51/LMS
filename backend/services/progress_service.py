...
def unmark_module_completed(db: Session, user_id: int, module_id: int) -> Progress:
    """Unmark a module as completed for a specific user."""
    # Verify user exists
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    try:
        ...
        if progress:
            # Remove the progress record entirely
            db.delete(progress)
            db.commit()
            return progress
        else:
            raise HTTPException(status_code=404, detail="No progress record found for this module")
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to unmark module as completed: {str(e)}")
