  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.data?.access_token) {
        ...
        const redirectPath = getLoginRedirectPath(from, user.role);
      ...
      if (err.response?.status === 401) {
        ...
        setError(err.response?.data?.detail || err.response?.data?.message || 'Unable to sign in.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AuthLayout title="Sign in" subtitle="Enter your credentials to access your account">
      ...
      <p className="mt-4 text-center text-sm text-[#6B7280]">
        ...
        <Link to="/register" className="text-[#2563EB] hover:underline font-medium">Sign up</Link>
      </p>
    </AuthLayout>
  );
};
